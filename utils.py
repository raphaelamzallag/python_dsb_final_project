...
def is_lockdown(X, date_column="date"):
    ...
    date_ranges = [
        {
            "start": datetime.datetime(2020, 3, 17),
...
def apply_couvre_feu(df, date_column="date"):
    couvre_feu_periods = [
        {
            "start": datetime.datetime(2020, 10, 17),
        ...
        {
            "start": datetime.datetime(2021, 3, 21),
        ...
        {
            "start": datetime.datetime(2021, 6, 10),
    ...
    def is_couvre_feu(date_heure):
        ...
        for period in couvre_feu_periods:
            if period["start"].date() <= dt.date() <= period["end"].date():
                if (dt.time() >= period["start_time"]) or (
                    dt.time() < period["end_time"]
                ):
                    return 1
        return 0
...
def prepro(X):
    ...
    data = is_jour_ferie(
        is_lockdown(create_x_weather(apply_couvre_feu(filter_columns(X))))
    )
